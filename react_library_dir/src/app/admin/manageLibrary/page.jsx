...
const ManageUser = () => {
  const fetchUserList = () => {
    axios
      .get("https://minor-backend-pearl.vercel.app/library/getall")
      .then((res) => {
  ...
  const deleteUser = (id) => {
    axios
      .delete("https://minor-backend-pearl.vercel.app/library/delete/" + id)
      .then((result) => {
  ...
  return (
    <div className="bg-gray-200 rounded-xl py-2 font-[sans-serif] ">
              ...
              {userList.map((user) => {
                return (
                  <tr key={user._id}>
                    ...
                    <td className="p-2 border via-violet-300">
                      <Link
                        href={`/admin/updateLibrary/` + user._id}
